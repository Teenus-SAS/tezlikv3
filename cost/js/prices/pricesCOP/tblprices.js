$(document).ready(function () {
    ...
    const loadAllData = async () => {
      try {
        ...
        op1 = typeCurrency === "2" && flag_currency_usd === "1" ? 2 :
              typeCurrency === "3" && flag_currency_eur === "1" ? 3 : 1;
    ...
    const loadTblPrices = (data, op, valueCoverage) => {
      ...
      $("#tblPrices").DataTable({
        ...
        columns: [
          { title: "No.", data: null, className: "uniqueClassName", render: (_, __, ___, meta) => meta.row + 1 },
          ...
          },
          {
            ...
            data: data => valueCoverage ? (data.price / valueCoverage).toFixed(2) : "",
            className: "classCenter",
            visible: op === 4,
            render: data => renderCost(data, op),
          },
          ...
          },
          {
            ...
            data: data => valueCoverage ? (data.sale_price / valueCoverage).toFixed(2) : "",
            className: "classCenter",
          ...
          },
          {
            ...
            render: data => data ? `<img src="${data}" alt="" style="width:50px;border-radius:100px">` : "",
          },
          {
            title: "Acciones",
            data: "id_product",
            className: "uniqueClassName",
            render: data => `<a href="/cost/details-prices"><i id="${data}" class="bi bi-zoom-in seeDetail" data-toggle='tooltip' title='Ficha de Costos' style="font-size: 30px;"></i></a>`,
          },
        ],
      });
    };
    ...
    const getPrice = (data, op, key) => {
      switch (op) {
        case 1: return parseFloat(data[key]); // COP
        case 3: return parseFloat(data[`${key}_eur`]); // EUR
        default: return parseFloat(data[`${key}_usd`]); // USD
      }
    };
    ...
    const renderCost = (data, op) => {
      if (!data) return "";
      const options = { maximumFractionDigits: op === 1 ? 0 : 2, minimumFractionDigits: 2 };
      return `$ ${parseFloat(data).toLocaleString("es-CO", options)}`;
    };
  
    const renderProfitability = data => {
      const { actualProfitability2 = 0, profitability, sale_price } = getDataCost(data);
      const text = `${actualProfitability2.toFixed(2)} %`;
      const badgeClass =
        actualProfitability2 < profitability && sale_price > 0 ? "badge badge-warning" :
        sale_price > 0 ? "badge badge-success" : "badge badge-primary";
