...
class CompaniesLicenseDao
{
    ...
    public function findCompanyLicenseActive()
    {
        ...
        $stmt = $connection->prepare("SELECT cp.id_company, cp.nit, cp.company, cl.license_start, cl.license_end, cl.quantity_user, cl.license_status, cl.inyection,
    ...
    public function addLicense($dataLicense, $id_company)
    {
        ...
        try {
            if (empty($dataLicense['license_start'])) {
                ...
                $stmt = $connection->prepare("INSERT INTO companies_licenses (id_company, license_start, license_end, quantity_user, license_status, plan, cost, planning, cost_price_usd, flag_employee, flag_composite_product, cost_historical, flag_indirect, inyection)
                                              VALUES (:id_company, :license_start, :license_end, :quantity_user, :license_status, :plan, :cost, :planning, :cost_price_usd, :flag_employee, :flag_composite_product, :cost_historical, :flag_indirect, :inyection)");
                $stmt->execute([
                ...
                $stmt = $connection->prepare("INSERT INTO companies_licenses (id_company, license_start, license_end, quantity_user, license_status, plan, cost_price_usd, flag_employee, flag_composite_product, cost_historical, flag_indirect, inyection)
                                          VALUES (:id_company, :license_start, :license_end, :quantity_user, :license_status, :plan, :cost_price_usd, :flag_employee, :flag_composite_product, :cost_historical, :flag_indirect, :inyection)");
                $stmt->execute([
                    'id_company' => $id_company,
    ...
    public function updateLicense($dataLicense)
    {
        $connection = Connection::getInstance()->getConnection();
        try {
            $stmt = $connection->prepare("UPDATE companies_licenses SET license_start = :license_start, license_end = :license_end, quantity_user = :quantity_user, inyection = :inyection,
