...
class GeneralUserAccessDao
{
    ...
    public function findUserAccessByUser($id_user)
    {
        ...
        $stmt = $connection->prepare("SELECT 
                                              ...
                                              IFNULL(cua.payroll_load, 0) AS payroll_load, IFNULL(cua.type_payroll, 0) AS type_payroll, IFNULL(cua.expense, 0) AS expense, IFNULL(cua.expense_distribution, 0) AS expense_distribution, IFNULL(cua.production_center, 0) AS production_center, IFNULL(cua.type_expense, 0) AS type_expense, IFNULL(cua.user, 0) AS cost_user, IFNULL(cua.backup, 0) AS cost_backup , IFNULL(cua.economy_scale, 0) AS cost_economy_scale, IFNULL(cua.multiproduct, 0) AS cost_multiproduct, IFNULL(cua.quote_payment_method, 0) AS quote_payment_method, 
                                              IFNULL(cua.quote_company, 0) AS quote_company, IFNULL(cua.quote_contact, 0) AS quote_contact, IFNULL(cua.price, 0) AS price, IFNULL(cua.custom_price, 0) AS custom_price, IFNULL(cua.type_custom_price, 0) AS type_custom_price, IFNULL(cua.analysis_material, 0) AS analysis_material, IFNULL(cua.simulator, 0) AS simulator, IFNULL(cua.historical, 0) AS historical, IFNULL(cua.support, 0) AS support, IFNULL(cua.quote, 0) AS quote, pa.cost_price AS plan_cost_price, cl.cost_price_usd AS plan_cost_price_usd, cl.flag_composite_product, pa.custom_price AS plan_custom_price, 
                                              cl.flag_production_center, cl.flag_employee, cl.flag_indirect, cl.inyection, pa.cost_analysis_material AS plan_cost_analysis_material, pa.cost_simulator AS plan_cost_simulator, cl.cost_historical AS plan_cost_historical, pa.cost_support AS plan_cost_support, pa.cost_quote AS plan_cost_quote, pa.cost_economy_scale AS plan_cost_economy_sale, pa.cost_multiproduct AS plan_cost_multiproduct, cl.flag_expense, cl.flag_family, cl.flag_type_price, u.firstname, u.lastname, u.email
                                      FROM users u
