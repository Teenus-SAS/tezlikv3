...
class GeneralUserAccessDao
{
    ...
    public function findUserAccessByUser($id_user)
    {
        ...
        $stmt = $connection->prepare("SELECT 
                                            IFNULL(cua.create_product, 0) AS cost_product, IFNULL(cua.create_materials, 0) AS cost_material, IFNULL(cua.create_machines, 0) AS cost_machine, IFNULL(cua.create_process, 0) AS cost_process, IFNULL(cua.product_materials, 0) AS cost_products_material, IFNULL(cua.product_process, 0) AS cost_products_process,  IFNULL(cua.factory_load, 0) AS factory_load,  IFNULL(cua.external_service, 0) AS external_service, IFNULL(cua.payroll_load, 0) AS payroll_load, IFNULL(cua.expense, 0) AS expense, IFNULL(cua.expense_distribution, 0) AS expense_distribution, IFNULL(cua.user, 0) AS cost_user, 
                                            IFNULL(cua.economy_scale, 0) AS cost_economy_scale, IFNULL(cua.multiproduct, 0) AS cost_multiproduct, IFNULL(cua.quote_payment_method, 0) AS quote_payment_method, IFNULL(cua.quote_company, 0) AS quote_company, IFNULL(cua.quote_contact, 0) AS quote_contact, IFNULL(cua.price, 0) AS price, IFNULL(cua.price_usd, 0) AS price_usd, IFNULL(cua.analysis_material, 0) AS analysis_material, IFNULL(cua.support, 0) AS support, IFNULL(cua.quote, 0) AS quote, IFNULL(pua.create_mold, 0) AS create_mold, IFNULL(pua.create_product, 0) AS planning_product, 
                                            IFNULL(pua.create_material, 0) AS planning_material, IFNULL(pua.create_machine, 0) AS planning_machine, IFNULL(pua.create_process, 0) AS planning_process, IFNULL(pua.products_material, 0) AS planning_products_material, IFNULL(pua.products_process, 0) AS planning_products_process, IFNULL(pua.programs_machine, 0) AS programs_machine, IFNULL(pua.cicles_machine, 0) AS cicles_machine, IFNULL(pua.inv_category, 0) AS inv_category, IFNULL(pua.sale, 0) AS sale, IFNULL(pua.user, 0) AS planning_user, IFNULL(pua.client, 0) AS client, 
                                            IFNULL(pua.orders_type, 0) AS orders_type, IFNULL(pua.inventory, 0) AS inventory, IFNULL(pua.plan_order, 0) AS plan_order, IFNULL(pua.program, 0) AS program, IFNULL(pua.plan_load, 0) AS plan_load, IFNULL(pua.explosion_of_material, 0) AS explosion_of_material, IFNULL(pua.office, 0) AS office, pa.cost_price AS plan_cost_price, cl.cost_price_usd AS plan_cost_price_usd, pa.cost_analysis_material AS plan_cost_analysis_material, pa.cost_support AS plan_cost_support, pa.cost_quote AS plan_cost_quote, 
                                            pa.cost_economy_scale AS plan_cost_economy_sale, pa.cost_multiproduct AS plan_cost_multiproduct, pa.plan_order AS plan_planning_order, pa.plan_inventory AS plan_planning_inventory, pa.plan_program AS plan_planning_program, pa.plan_load AS plan_planning_load, pa.plan_explosion_of_material AS plan_planning_explosion_of_material, pa.plan_office AS plan_planning_office, cl.flag_expense, u.firstname, u.lastname, u.email
                                      FROM users u
