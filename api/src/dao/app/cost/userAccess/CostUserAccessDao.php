...
class CostUserAccessDao
{
    ...
    public function findAllUsersAccess($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT us.id_user, us.firstname, us.lastname, us.email, IFNULL(usa.create_product, 0) AS create_product, IFNULL(usa.create_materials, 0) AS create_materials, IFNULL(usa.create_machines, 0) AS create_machines, IFNULL(usa.create_process, 0) AS create_process, IFNULL(usa.product_materials, 0) AS product_materials, 
                                                 IFNULL(usa.factory_load, 0) AS factory_load, IFNULL(usa.external_service, 0) AS external_service, IFNULL(usa.payroll_load, 0) AS payroll_load, IFNULL(usa.type_payroll, 0) AS type_payroll, IFNULL(usa.expense, 0) AS expense, IFNULL(usa.type_expense, 0) AS type_expense, IFNULL(usa.user, 0) AS user, IFNULL(usa.backup, 0) AS backup, 
    ...
    public function findUserAccess($id_company, $id_user)
    {
        ...
        $stmt = $connection->prepare(
            "SELECT us.id_user, us.firstname, us.lastname, us.email, IFNULL(usa.create_product, 0) AS create_product, IFNULL(usa.create_materials, 0) AS create_materials, IFNULL(usa.create_machines, 0) AS create_machines, IFNULL(usa.create_process, 0) AS create_process, IFNULL(usa.product_materials, 0) AS product_materials, 
                    IFNULL(usa.factory_load, 0) AS factory_load, IFNULL(usa.external_service, 0) AS external_service, IFNULL(usa.payroll_load, 0) AS payroll_load, IFNULL(usa.type_payroll, 0) AS type_payroll, IFNULL(usa.expense, 0) AS expense, IFNULL(usa.user, 0) AS user, IFNULL(usa.backup, 0) AS backup, 
                    IFNULL(usa.economy_scale, 0) AS economy_scale, IFNULL(usa.multiproduct, 0) AS multiproduct, IFNULL(usa.quote_payment_method, 0) AS quote_payment_method, IFNULL(usa.quote_company, 0) AS quote_company, IFNULL(usa.quote_contact, 0) AS quote_contact, 
    ...
    public function insertUserAccessByUser($dataUser, $typeCustomPrice)
    {
        ...
        try {
            $stmt = $connection->prepare("INSERT INTO cost_users_access (id_user, create_product, create_materials, create_machines, create_process, product_materials,  
                                                                         factory_load, external_service, payroll_load, type_payroll, expense, type_expense, user, backup, economy_scale, multiproduct,
            ...
            $stmt->execute([
                ...
                'type_expense' => $dataUser['typeExpenses'],                    'historical' => $dataUser['historical'],
                'user' => $dataUser['costUser'],                                'support' => $dataUser['support'],
                'backup' => $dataUser['costBackup'],                            'quote' => $dataUser['quote'],
                'economy_scale' => $dataUser['economyScale'],                   'type_payroll' => $dataUser['typePayroll'],
            ]);
            $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
        } catch (\Exception $e) {
    ...
    public function updateUserAccessByUsers($dataUser, $typeCustomPrice)
    {
        ...
        if ($rows > 1) {
            ...
            try {
                $stmt = $connection->prepare("UPDATE cost_users_access SET create_product = :create_product, create_materials = :create_materials, create_machines = :create_machines, create_process = :create_process, product_materials = :product_materials, type_payroll = :type_payroll,
                                                                           factory_load = :factory_load, external_service = :external_service, payroll_load = :payroll_load, expense = :expense, type_expense = :type_expense, user = :user, backup = :backup, economy_scale = :economy_scale, multiproduct = :multiproduct, 
