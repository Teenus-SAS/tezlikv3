...
class GeneralExpenseDistributionAnualDao
{
    private $logger;
    ...
    public function findAllProductsNotInEDistribution($id_company)
    {
        ...
        //                               AND p.active = 1");
        $stmt = $connection->prepare("SELECT p.id_product, p.reference, p.product, p.composite, IFNULL(ed.id_expense_distribution_anual, 0) AS status, pc.new_product,
                                      ...
                                      FROM products p
                                        LEFT JOIN expenses_distribution_anual ed ON ed.id_product = p.id_product
