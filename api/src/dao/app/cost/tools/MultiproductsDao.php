...
class MultiproductsDao
{
    ...
    public function findAllMultiproducts($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT p.id_product, p.product, pc.price, IFNULL((SELECT SUM(expense_value) FROM expenses WHERE id_company = p.id_company), 0) AS expense, 
                                             (pc.cost_workforce + IFNULL((IF(mp.units_sold IS NULL,(SELECT SUM(e.expense_value) FROM expenses e INNER JOIN puc pu ON e.id_puc = pu.id_puc WHERE e.id_company = p.id_company 
                                             ...
                                             WHERE e.id_company = p.id_company AND (pu.number_count LIKE '51%' OR pu.number_count LIKE '52%' OR pu.number_count LIKE '53%')) * (mp.participation / 100))), 0)) AS cost_fixed, 
