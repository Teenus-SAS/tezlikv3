    ...
    public function findAllMultiproducts($id_company)
    {
        ...
        try {
            $stmt = $connection->prepare("SELECT p.id_product, p.product, pc.price, IFNULL((SELECT SUM(expense_value) FROM expenses WHERE id_company = p.id_company), 0) AS expense, 
                                                 ((SELECT IFNULL(SUM(salary_net), 0) FROM (SELECT salary_net FROM payroll WHERE id_company = 6 GROUP BY employee) AS payroll)) AS cost_fixed,
            ...
            for ($i = 0; $i < sizeof($multiproducts); $i++) {
                $stmt = $connection->prepare("SELECT IFNULL(SUM(salary_net), 0) AS salary_net FROM (SELECT py.salary_net 
                                          FROM payroll py
                                            INNER JOIN products_process pp ON pp.id_process = py.id_process
                                          WHERE py.id_company = :id_company AND pp.id_product = :id_product
                                          GROUP BY employee) AS payroll");
                $stmt->execute([
                    'id_company' => $id_company,
                    'id_product' => $multiproducts[$i]['id_product']
                ]);
                $payroll = $stmt->fetch($connection::FETCH_ASSOC);
                $multiproducts[$i]['cost_fixed'] = $payroll['salary_net'];
            }
