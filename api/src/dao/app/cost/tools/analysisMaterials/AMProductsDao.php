    ...
    public function findConsolidatedRawMaterialsByProduct($products, $id_company)
    {
        ...
        try {
            ...
            $stmt = $connection->prepare("SELECT pm.id_product_material, pm.id_product, p.reference AS reference_product, p.product, m.id_material, m.reference AS reference_material, m.material, 
                                                 ...
                                                 (((SELECT SUM(quantity) FROM products_materials WHERE id_product IN($products) AND id_material = pm.id_material)*m.cost)/ (SELECT SUM(pm.quantity*m.cost) 
                                                 FROM products_materials pm INNER JOIN materials m ON m.id_material = pm.id_material WHERE pm.id_product = p.id_product AND pm.id_company = p.id_company))*100 AS participation 
                                          FROM products p 
                                            INNER JOIN products_materials pm ON pm.id_product = p.id_product
                                            INNER JOIN materials m ON m.id_material = pm.id_material
                                            INNER JOIN convert_units u ON u.id_unit = pm.id_unit
                                            WHERE pm.id_product IN ($products) AND pm.id_company = :id_company GROUP BY PM.id_material  
