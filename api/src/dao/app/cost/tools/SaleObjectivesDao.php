...
class SaleObjectivesDao
{
    ...
    public function findAllProductsByCompany($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT p.id_product, p.reference, p.product, pc.sale_price, pc.profitability, pc.commission_sale, pc.price, p.img, IFNULL(IFNULL(ed.turnover, 0) / IFNULL(ed.units_sold, 0), 0) AS real_price
                                      FROM products p
