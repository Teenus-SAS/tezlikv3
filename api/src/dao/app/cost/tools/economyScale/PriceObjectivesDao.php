...
class PriceObjectivesDao
{
    ...
    public function findAllProductsByCompany($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT p.id_product, p.reference, p.product, pc.sale_price, pc.profitability, pc.commission_sale, pc.price, p.img, IFNULL(ed.turnover / ed.units_sold, 0) AS real_price, 
                                             ...
                                             IFNULL(po.price_1, 0) AS price_1, IFNULL(po.price_2, 0) AS price_2, IFNULL(po.price_3, 0) AS price_3, IFNULL(po.min_profitability, 0) AS min_profitability, IFNULL(po.max_profitability, 0) AS max_profitability
                                      FROM products p
    ...
    public function insertPriceObjectiveByCompany($dataPrice, $id_company)
    {
        try {
            $connection = Connection::getInstance()->getConnection();
            $stmt = $connection->prepare("INSERT INTO price_objectives (id_company, id_product, profitability, min_profitability, max_profitability, unit_1, unit_2, unit_3, price_1, price_2, price_3) 
                                          VALUES (:id_company, :id_product, :profitability, :min_profitability, :max_profitability, :unit_1, :unit_2, :unit_3, :price_1, :price_2, :price_3)");
            $stmt->execute([
                'id_company' => $id_company,
                'profitability' => $dataPrice['profitability'],
                'min_profitability' => $dataPrice['min_profitability'],
                'max_profitability' => $dataPrice['max_profitability'],
    ...
    public function updatePriceObjective($dataPrice)
    {
        try {
            $connection = Connection::getInstance()->getConnection();
            $stmt = $connection->prepare("UPDATE price_objectives SET profitability = :profitability, min_profitability = :min_profitability, max_profitability = :max_profitability, unit_1 = :unit_1, unit_2 = :unit_2, unit_3 = :unit_3, price_1 = :price_1, price_2 = :price_2, price_3 = :price_3
