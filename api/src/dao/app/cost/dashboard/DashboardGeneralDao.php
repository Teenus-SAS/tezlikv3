...
class DashboardGeneralDao
{
  ...
  public function findTotalMultiproducts($id_company)
  {
    ...
    $stmt = $connection->prepare("SELECT (((IFNULL((SELECT SUM(expense_value) FROM expenses WHERE id_company = p.id_company), 0)) + (IFNULL(SUM((pc.cost_workforce) + IF(mp.units_sold IS NULL, (SELECT SUM(e.expense_value) FROM expenses e INNER JOIN puc pu ON pu.id_puc = e.id_puc 
                                         WHERE e.id_company = p.id_company AND (pu.number_count LIKE '51%' OR pu.number_count LIKE '52%' OR pu.number_count LIKE '53%')),((SELECT SUM(e.expense_value) FROM expenses e INNER JOIN puc pu ON pu.id_puc = e.id_puc WHERE e.id_company = 
                                         p.id_company AND (pu.number_count LIKE '51%' OR pu.number_count LIKE '52%' OR pu.number_count LIKE '53%')) * (mp.participation / 100)))), 0))) / (IFNULL(SUM(((pc.price) - (pc.cost_materials + pc.cost_indirect_cost + 
                                         ((pc.commission_sale / 100) * pc.price) + (SELECT IFNULL(SUM(cost), 0) FROM services WHERE id_product = p.id_product))) * (mp.participation / 100)), 0))) AS totalUnits
                                  FROM products p
