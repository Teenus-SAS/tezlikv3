...
class Planning_machinesDao
{
    ...
    public function findAllPlanMachines($id_company)
    {
        ...
        $stmt = $connection->prepare("SELECT pm.id_program_machines, m.machine, pm.number_workers, pm.hours_day, pm.hour_start, pm.hour_end, pm.year, pm.january, pm.february,
    ...
    public function updatePlanMachines($dataPMachines)
    {
        ...
        try {
            $stmt = $connection->prepare("UPDATE plan_program_machines SET id_machine = :id_machine, number_workers = :number_workers, hours_day = :hours_day, hour_start = :hour_start, hour_end = :hour_end, 
                                          ...
                                          WHERE id_program_machines = :id_program_machines");
            $stmt->execute([
    ...
    public function deletePlanMachines($id_program_machines)
    {
        $connection = Connection::getInstance()->getConnection();

        $stmt = $connection->prepare("SELECT * FROM plan_program_machines WHERE id_program_machines = :id_program_machines");
        $stmt->execute(['id_program_machines' => $id_program_machines]);
        $rows = $stmt->rowCount();

        if ($rows > 0) {
            $stmt = $connection->prepare("DELETE FROM plan_program_machines WHERE id_program_machines = :id_program_machines");
            $stmt->execute(['id_program_machines' => $id_program_machines]);
            $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
        }
    }
