...
$app->group('/dataSheetMaterials', function (RouteCollectorProxy $group) {

    $group->get('/productsMaterials/{idProduct}', function (Request $request, Response $response, $args) {
        // session_start();
    ...
    $group->get('/allProductsMaterials', function (Request $request, Response $response, $args) {
    ...
    $group->post('/productsMaterialsDataValidation', function (Request $request, Response $response, $args) {
        ...
        if (isset($dataProductMaterial)) {
            ...
            if (isset($dataProductMaterial['debugg']))
                $debugg = $dataProductMaterial['debugg'];
            else $debugg = [];
            ...
            for ($i = 0; $i < sizeof($productMaterials); $i++) {
                ...
                if (!$magnitude) {
                    $row = $i + 2;
                    array_push($debugg, array('message' => "Magnitud no existe en la base de datos. Fila: $row"));
                } else {
                    ...
                    if (!$unit) {
                        $row = $i + 2;
                        array_push($debugg, array('message' => "Unidad no existe en la base de datos. Fila: $row"));
                    }
                }
    ...
    $group->post('/addProductsMaterials', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
        ...
        if ($dataProductMaterials > 1) {
            ...
            for ($i = 0; $i < sizeof($productMaterials); $i++) {
                ...
                if (strtoupper(trim($productMaterials[$i]['type'])) == 'MATERIAL') {
                    $findProductsMaterials = $productsMaterialsDao->findProductMaterial($productMaterials[$i], $id_company);
    ...
    $group->post('/updateProductsMaterials', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
        $coverage_usd = $_SESSION['coverage_usd'];
        $dataProductMaterial = $request->getParsedBody();
    ...
    $group->post('/deleteProductMaterial', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
        $coverage_usd = $_SESSION['coverage_usd'];
        $dataProductMaterial = $request->getParsedBody();
