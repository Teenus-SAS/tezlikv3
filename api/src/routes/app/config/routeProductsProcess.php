...
$app->group('/dataSheetProcess', function (RouteCollectorProxy $group) {

    // Productos procesos
    $group->get('/productsProcess/{idProduct}', function (Request $request, Response $response, $args) {
        // session_start();
    ...
    $group->get('/allProductsProcess', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
    ...
    $group->get('/employees/{id_product_process}', function (Request $request, Response $response, $args) {
    ...
    $group->post('/productsProcessDataValidation', function (Request $request, Response $response, $args) {
    ...
    $group->post('/addProductsProcess', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
        ...
        if ($dataProductsProcess > 1) {
            ...
            if (!$productProcess) {
                ...
                if ($productProcess == null) {
                    $dataPayroll = $costWorkforceDao->sumTotalCostPayroll($dataProductProcess['idProduct'], $id_company);
                ...
                if ($productProcess == null && isset($data['totalPrice']) && $_SESSION['flag_currency_usd'] == '1') {
                    // Convertir a Dolares 
                    $k = [];
            ...
            for ($i = 0; $i < sizeof($productProcess); $i++) {
                ...
                // Calcular costo nomina total
                if ($resolution == null) {
                    $dataPayroll = $costWorkforceDao->sumTotalCostPayroll($productProcess[$i]['idProduct'], $id_company);
    ...
    $group->post('/updateProductsProcess', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
        $coverage_usd = $_SESSION['coverage_usd'];
        $dataProductProcess = $request->getParsedBody();
    ...
    $group->post('/saveRouteProductProcess', function (Request $request, Response $response, $args) {
        // session_start();
    ...
    $group->post('/deleteProductProcess', function (Request $request, Response $response, $args) {
        // session_start();
        $id_company = $_SESSION['id_company'];
        $coverage_usd = $_SESSION['coverage_usd'];
        $dataProductProcess = $request->getParsedBody();
