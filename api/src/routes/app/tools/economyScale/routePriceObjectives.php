...
$app->group('/objetivesPrices', function (RouteCollectorProxy $group) use ($priceObjectivesDao) {

    $group->get('', function (Request $request, Response $response, $args) use ($priceObjectivesDao) {
    ...
    $group->post('/savePriceObjectives', function (Request $request, Response $response, $args) use ($priceObjectivesDao) {
        $id_company = $_SESSION['id_company'];
