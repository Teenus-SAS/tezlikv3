...
$app->get('/benefits', function (Request $request, Response $response, $args) use ($benefitsDao) {
    $benefits = $benefitsDao->findAllBenefits();
    $response->getBody()->write(json_encode($benefits));
    return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
})->add(new SessionMiddleware());

$app->post('/updateBenefit', function (Request $request, Response $response, $args) use ($benefitsDao) {
