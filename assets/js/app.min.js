  ...
  (function () {
    ...
    $('#layout').on('click', function () {
      ...
      $('body').on('click', function (e) {
        0 < $(e.target).closest('.side-navbar').length ||
          0 < $(e.target).closest('[data-sidebar="mobile"]').length ||
          ($('body').removeClass('show-sidebar'),
          $('[data-sidebar="mobile"]').length &&
      ...
      $('body').hasClass('horizontal-navbar') ||
        (document.getElementById('metismenu') &&
      ...
      document.getElementById('notify-scrollbar') &&
      ...
      $('.layout-nav .side-nav-link').on('click', function () {
        ...
        $('body')
          .removeClass()
          .addClass('pace-done ' + e);
        var t = document.body.classList.value.trim();
        localStorage.setItem('classes', t);
      }),
      $('#full-screen').on('click', function () {
        $(this).children().toggleClass('bx-fullscreen bx-exit-fullscreen'),
          (e = e ? exitFullScreen(e) : fullScreen(e));
      }),
      $('#vertical-menu-btn').on('click', function (e) {
        if (
          (e.preventDefault(), e.stopPropagation(), 1024 < $(window).width())
        ) {
          $('body').hasClass('left-side-menu-condensed')
            ? $('body').removeClass('left-side-menu-condensed')
            : $('body').addClass('left-side-menu-condensed');
          var t = document.body.classList.value.trim();
          localStorage.setItem('classes', t);
        } else $('body').toggleClass('show-sidebar');
      }),
      $('#countries .dropdown-item').on('click', function () {
        var e = $(this).children('img').attr('src'),
          t = $(this).children('span').attr('data-lang');
        (t = t.toUpperCase()),
          $('#page-header-country-dropdown').children('img').attr('src', e),
          $('#page-header-country-dropdown').children('span').text(t);
      }),
      $('.floating-label .form-control').on('change', function () {
        '' !== $(this).val()
          ? $(this).parents('.floating-label').addClass('enable-floating-label')
          : $(this).parent().hasClass('show-label') ||
            $(this)
